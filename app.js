import {
  ...
  saveRoundResults, nextRound, setStatus, finishGame, restartGame, MAX_PLAYERS
} from "./room.js";
...
function renderFinished(room) {
  ...
  sorted.forEach(([pid, p], idx) => {
    ...
    const medal = idx === 0 ? "🏆 " : "";
    li.innerHTML = `<span class="player-dot" style="background:${p.color}"></span>
      <span class="player-name">${medal}${p.name}</span><span class="player-score">${p.score || 0}</span>`;
