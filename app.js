import {
  ...
  restartGame, restartWithSameCategories, overrideValidation, MAX_PLAYERS
} from "./room.js";
...
function showScreen(id) {
  ...
  document.getElementById("btn-global-leave").classList.toggle("hidden", id === "screen-home" || id === "screen-setup");
...
async function processRoundAsHost() {
  try {
    ...
    const { validation, needsReview } = await validateBatch(letter, itemsToCheck);
    ...
    if (needsReview.length > 0) {
      toast(`${needsReview.length} resposta(s) não puderam ser validadas pela IA — revise no placar.`);
    }

    await saveRoundResults(state.roomCode, validation, roundScores, updatedTotals, needsReview);
  } catch (err) {
    console.error(err);
...
function renderScoreboard(room) {
  ...
  const needsReview = room.round.needsReview || [];
  ...
  Object.entries(room.players).forEach(([pid, p]) => {
    ...
    const flagged = needsReview.includes(key);
    const statusClass = !answer || answer === "(em branco)" || !isValid ? "pts-invalida" : "pts-unica";

    let reviewHtml = "";
    if (flagged && state.isHost) {
      reviewHtml = `
        <p class="footnote" style="color:var(--danger)">Não validada pela IA (sem chave configurada ou erro na chamada) — confira e ajuste:</p>
        <div class="stack" style="flex-direction:row; gap:8px;">
          <button class="btn btn-secondary btn-review-valid" data-pid="${pid}" data-category="${category}" data-valid="true">Marcar válida</button>
          <button class="btn btn-secondary btn-review-valid" data-pid="${pid}" data-category="${category}" data-valid="false">Marcar inválida</button>
        </div>`;
    } else if (flagged) {
      reviewHtml = `<p class="footnote">Aguardando o host revisar essa resposta.</p>`;
    }
  ...
  details.querySelectorAll(".btn-review-valid").forEach((btn) => {
    btn.onclick = async () => {
      await applyManualReview(room, btn.dataset.pid, btn.dataset.category, btn.dataset.valid === "true");
    };
  });
...
async function applyManualReview(room, pid, category, isValid) {
  const answers = room.round.answers || {};
  const oldValidation = room.round.validation || {};
  const oldRoundScores = room.round.roundScores || {};
  const oldNeedsReview = room.round.needsReview || [];

  const key = `${pid}::${category}`;
  const newValidation = { ...oldValidation, [key]: isValid };
  const newNeedsReview = oldNeedsReview.filter((k) => k !== key);

  const playerIds = Object.keys(room.players);
  const { roundScores: newRoundScores } = computeScores(answers, newValidation, [category], playerIds);

  await overrideValidation(state.roomCode, newValidation, newRoundScores, oldRoundScores, newNeedsReview);
  toast("Resposta revisada e placar atualizado.");
}
