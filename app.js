import {
  ...
  saveRoundResults, nextRound, setStatus, finishGame, forfeitGame, restartGame, MAX_PLAYERS
} from "./room.js";
...
document.getElementById("btn-start-game").onclick = async () => {
  ...
  const totalRounds = parseInt(document.getElementById("input-total-rounds").value, 10) || 5;

  await updateSettings(state.roomCode, { categories, timerMode, timerSeconds, totalRounds });
  ...
  await startPickingPhase(state.roomCode, playerIds, 1);
};
...
document.getElementById("btn-next-round").onclick = async () => {
  ...
  if (currentRoundNumber >= totalRounds) {
    ...
    await nextRound(state.roomCode, playerIds, currentRoundNumber + 1);
  }
};
