...
// ---------- Limite de jogos grátis / desbloqueio ----------
export const FREE_GAMES_LIMIT = 6;

/** ID persistente do aparelho, gerado uma vez e guardado no localStorage. */
export function getDeviceId() {
  let id = localStorage.getItem("remember_device_id");
  if (!id) {
    const rand = Math.random().toString(36).slice(2, 10) + Math.random().toString(36).slice(2, 6);
    id = "d_" + rand.slice(0, 12);
    localStorage.setItem("remember_device_id", id);
  }
  return id;
}

/** Lê quantos jogos (como host) esse aparelho já criou e se está desbloqueado. */
export async function getPaywallStatus(deviceId) {
  const snap = await get(ref(db, `devices/${deviceId}`));
  const data = snap.val() || { gamesCreated: 0, unlocked: false };
  const gamesCreated = data.gamesCreated || 0;
  const unlocked = !!data.unlocked;
  return {
    gamesCreated,
    unlocked,
    blocked: !unlocked && gamesCreated >= FREE_GAMES_LIMIT
  };
}

/** Soma +1 ao contador de partidas criadas como host desse aparelho. */
export async function incrementGamesCreated(deviceId) {
  const deviceRef = ref(db, `devices/${deviceId}`);
  await runTransaction(deviceRef, (current) => {
    const gamesCreated = (current && current.gamesCreated) || 0;
    return { ...current, gamesCreated: gamesCreated + 1, unlocked: (current && current.unlocked) || false };
  });
}

/** Tenta resgatar um código de desbloqueio. Lança erro se o código for inválido/já usado. */
export async function redeemUnlockCode(deviceId, rawCode) {
  const code = rawCode.trim().toUpperCase();
  if (!code) throw new Error("Digite o código.");

  const codeRef = ref(db, `unlock-codes/${code}`);
  const snap = await get(codeRef);
  if (!snap.exists()) throw new Error("Código inválido.");
  if (snap.val().used) throw new Error("Esse código já foi usado.");

  await update(codeRef, { used: true, deviceId });
  await update(ref(db, `devices/${deviceId}`), { unlocked: true, lastRedeemedCode: code });
}
